...
void cConfiguration::WriteConfigurationToFile(const std::string &filePath)
{
    ...
    for (std::vector<c_configuration_checker_bundle *>::const_iterator it = m_checkerBundles.begin();
...
void cConfiguration::ProcessDomNode(DOMNode *const nodeToProcess, cConfiguration *const cConfig) const
{
    if (nodeToProcess->getNodeType() == DOMNode::ELEMENT_NODE)
    {
        ...
        else if (Equals(currentTagName, XMLString::transcode(c_configuration_checker_bundle::TAG_CHECKERBUNDLE)))
        {
            c_configuration_checker_bundle *p_checkerBundle =
                c_configuration_checker_bundle::ParseConfigurationCheckerBundle(nodeToProcess, currentIssueElement);
...
{
    for (std::vector<c_configuration_checker_bundle *>::iterator it = m_checkerBundles.begin();
         it != m_checkerBundles.end(); it++)
    {
        (*it)->Clear();
        delete (*it);
...
std::vector<c_configuration_checker_bundle *> cConfiguration::GetCheckerBundles() const
{
...
c_configuration_checker_bundle *cConfiguration::GetCheckerBundleByName(const std::string &applicationName)
{
    std::vector<c_configuration_checker_bundle *>::iterator it = m_checkerBundles.begin();
...
c_configuration_checker_bundle *cConfiguration::AddCheckerBundle(const std::string &applicationName)
{
    c_configuration_checker_bundle *result = new c_configuration_checker_bundle(applicationName);
...
bool cConfiguration::AddConfiguration(const cConfiguration *pAnotherConfig)
{
    ...
    std::vector<c_configuration_checker_bundle *> checkerBundleConfigs = pAnotherConfig->GetCheckerBundles();
    std::vector<c_configuration_checker_bundle *>::iterator itCheckerBundle = checkerBundleConfigs.begin();

    for (; itCheckerBundle != checkerBundleConfigs.end(); itCheckerBundle++)
    {
        c_configuration_checker_bundle *newCheckerBundle =
            AddCheckerBundle((*itCheckerBundle)->GetCheckerBundleApplication());
        newCheckerBundle->OverwriteParams((*itCheckerBundle)->GetParams());

        // Add Checker configs
        std::vector<c_configuration_checker *> checkerConfigs = (*itCheckerBundle)->GetCheckers();
        std::vector<c_configuration_checker *>::iterator itChecker = checkerConfigs.begin();

        for (; itChecker != checkerConfigs.end(); itChecker++)
        {

            c_configuration_checker *newChecker = newCheckerBundle->AddChecker(
