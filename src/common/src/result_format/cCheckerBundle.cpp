...
cCheckerBundle::cCheckerBundle(const std::string& checkerName, const std::string& checkerSummary, const std::string& description) :
...
DOMElement * cCheckerBundle::WriteXML(DOMDocument* pResultDocument)
{
    ...
    // Add checkers
    for (std::list<cChecker*>::const_iterator it = m_Checkers.begin(); it != m_Checkers.end();
         ++it)
    {
...
cCheckerBundle* cCheckerBundle::ParseFromXML(DOMNode* pXMLNode, DOMElement* pXMLElement, cResultContainer* targetContainer)
{
    ...
    for (XMLSize_t j = 0; j < checkerNodeCount; ++j)
    {
        ...
        if (currentCheckerNode->getNodeType() == DOMNode::ELEMENT_NODE)
        {
            ...
            {
                cChecker* checkerInstance = cChecker::ParseFromXML(currentCheckerNode, currentIssueElement, checkerBundle);
...
// Adds a new checker
cChecker* cCheckerBundle::CreateChecker(const std::string& checkerId, const std::string& strDescription, const std::string& strSummary)
{
    return CreateChecker(new cChecker(checkerId, strDescription, strSummary));
}

cChecker* cCheckerBundle::CreateCheckerWithIssues(const std::string& strCheckerId, const std::string& strDescription, eIssueLevel issueLevel, std::map<std::string, std::string> m_Issues)
{
...
unsigned long long cCheckerBundle::NextFreeId() const
{
    ...
    else
    {
        ...
        while (true)
        {
            ...
            for (std::list<cIssue*>::const_iterator itIssues = issues.cbegin();
                 itIssues != issues.cend();
                 itIssues++)
            {
...
{
    for (std::list<cChecker*>::const_iterator itChecker = m_Checkers.begin();
         itChecker != m_Checkers.end();
         itChecker++)
    {
        std::list<cIssue*> issues = (*itChecker)->GetIssues();

        for (std::list<cIssue*>::const_iterator itIssue = issues.begin(); itIssue != issues.end();
             itIssue++)
...
{
    for (std::list<cChecker*>::const_iterator itCheckers = m_Checkers.cbegin();
         itCheckers != m_Checkers.cend();
         itCheckers++)
    {
...
std::list<cIssue*> cCheckerBundle::GetIssues() const
{
    ...
    for (std::list<cChecker*>::const_iterator itCheckers = checkers.cbegin();
         itCheckers != checkers.cend();
         itCheckers++)
    {
